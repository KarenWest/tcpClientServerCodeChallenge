...
void onIncomingMsg(const std::string &clientIP, const char * msg, size_t size) {
    ...
    if (msgStr.front() == '1')
        std::cout << "Observer got client1 msg: " << msgStr << "\n";
    else if (msgStr.front() == '2')
        std::cout << "Observer got client2 msg: " << msgStr << "\n";
    else if (msgStr.front() == '3')
        std::cout << "Observer got client3 msg: " << msgStr << "\n";
}
...
// observer callback. will be called for every new message received by clients
// with the requested IP address
//void onIncomingMsg3(const std::string &clientIP, const char * msg, size_t size) {
//    std::string msgStr = msg;
    // print client message
//    if (msgStr.front() == '3')
//        std::cout << "Observer3 got client msg: " << msgStr << "\n";
//}
...
bool handleMenuSelection(int selection) {
    ...
    switch (selection) {
        ...
        case 4: {
	    server.printClientMsgsByPriorityOfId();
            break;
        }
        case 5: { // close server
...
int main()
{
    ...
    //observer2.wantedIP = "127.0.0.1"; // use empty string instead to receive messages from any IP address
    //server.subscribe(observer2);

    // configure and register observer3
    //observer3.incomingPacketHandler = onIncomingMsg3;
    //observer3.disconnectionHandler = nullptr; // nullptr or not setting this means we don't care about disconnection event
    //observer3.wantedIP = "127.0.0.1"; // use empty string instead to receive messages from any IP address
    //server.subscribe(observer3);
